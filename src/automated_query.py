...
def find_key_get_value(query, output, maxmin, key, nodup, topx):
  try:
    ...
    for ele in itemlist :
      if ele.attributes['k'].value==key:
        ...
        for rep in replaced:
          ...
          counter+=1
          if counter==topx: return parse_to_string(collect_answer)
    if nodup: return parse_to_string(list(set(collect_answer)))
    if maxmin==1:
      return max(collect_answer)
    elif maxmin==2:
      return min(collect_answer)
    return parse_to_string(collect_answer)
  except:
...
def at_least_x(query, output, topx):
  try:
    xmldoc = minidom.parseString(output)
    total = 0
    itemlist = xmldoc.getElementsByTagName('node')
...
def get_lat_lon(query, output, topx):
  try:
    xmldoc = minidom.parseString(output)
    itemlist = xmldoc.getElementsByTagName('node') 
    latlon = ""
...
def execute_query(count, query, type, topx=0, key="", nodup=False, maxmin=0):
  ...
  if type==1:
    return find_key_get_value(query, output, maxmin, key, nodup, topx).encode('utf-8')
  elif type==2:
    return count_nodes(query, output)
  elif type==3:
    return at_least_x(query, output, topx)
  elif type==4:
    return get_lat_lon(query, output, topx).encode('utf-8')
  else:
    return ""